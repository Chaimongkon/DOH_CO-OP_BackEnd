...
export async function DELETE(request: Request) {
  try {
    ...
    const [rows] = await connection.query<any[]>(
      "SELECT ImagePath FROM particles WHERE Id = ?",
      [id]
    );
    connection.release();
    ...
    const images = JSON.parse(rows[0].ImagePath) as string[];
    ...
    const [updateResult] = await connection.query<any>(
      "UPDATE particles SET ImagePath = ? WHERE Id = ?",
      [JSON.stringify(images), id]
    );

    if (updateResult.affectedRows === 0) {
      return NextResponse.json(
        { message: "Failed to update image list" },
        { status: 500 }
      );
    }
