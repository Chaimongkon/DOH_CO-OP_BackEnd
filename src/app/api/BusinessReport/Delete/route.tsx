...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const querySelect = "SELECT ImagePath, FilePath FROM businessreport WHERE Id = ?";
    ...
    const { ImagePath, FilePath } = rows[0]; // Access the first row
    ...
    try {
      ...
      if (FilePath) {
        const pdfPathFull = path.join(process.cwd(), "public", FilePath); // Resolve full PDF path
        await fs.unlink(pdfPathFull); // Remove the PDF file
      }
    } catch (fileError) {
    ...
    const queryDelete = "DELETE FROM businessreport WHERE Id = ?";
