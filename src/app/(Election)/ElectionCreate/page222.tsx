...
const generateFieldsAndSequence = (totalRows: number) => {
  const result = [];
  let field = 1; // เริ่มต้น FieldNumber
  let sequence = 1; // เริ่มต้น SequenceNumber

  for (let rowCount = 0; rowCount < totalRows; rowCount++) {
    result.push({
      FieldNumber: field, // เพิ่ม FieldNumber ตามลำดับ
      SequenceNumber: sequence, // เพิ่ม SequenceNumber ตามลำดับ
    });

    // เพิ่มค่า FieldNumber และ SequenceNumber
    field++;
    sequence++;

    // รีเซ็ต FieldNumber หากเกินค่าที่กำหนด
    if (field > fieldNumber) {
      field = 1;
    }

    // รีเซ็ต SequenceNumber หากเกินค่าที่กำหนด
  ...
  const handleSubmit = async () => {
    ...
    const dataWithFieldsAndSequence = excelData.map((row, index) => {
      ...
      const field = (index % fieldNumber) + 1; // FieldNumber จะวนตามจำนวน fieldNumber
      ...
      return {
        ...
        Department,
        MemberType,
        FieldNumber: "ช่อง " + field, // Format FieldNumber
